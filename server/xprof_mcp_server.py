        ...
        types.PromptMessage(
            ...
            content=types.TextContent(
                ...
                text=(
                    ...
                    "### Kernel Profiling / LLO Workflow (Pallas/Mosaic kernels)\n\n"
                    "For INSIDE-the-kernel analysis (which functional unit is the\n"
                    "bottleneck), capture with LIBTPU_INIT_ARGS=\n"
                    "'--xla_enable_custom_call_region_trace=true\n"
                    " --xla_xprof_register_llo_debug_info=true' then:\n\n"
                    "1. `check_kernel_profiling(run)` — ALWAYS FIRST: were the flags\n"
                    "   on? Analyzing a flagless trace at LLO level = silent noop.\n"
                    "2. `list_kernel_invocations(run)` — kernel handles + durations.\n"
                    "3. `get_llo_utilization(run, kernel=...)` — per-unit % util\n"
                    "   (MXU / Scalar ALU / Vector ALU / Vector Load / Store / XLU)\n"
                    "   + bottleneck verdict. Vector Load >> MXU = memory-bound;\n"
                    "   Scalar ALU > MXU = scalar serialization; Spills > 0 =\n"
                    "   register pressure.\n"
                    "4. `get_kernel_stage_breakdown(run, kernel=...)` — Mosaic ep_*\n"
                    "   stage times; wait_ratio > ~0.3 = DMA-starved compute (add\n"
                    "   pipeline buffers / enlarge blocks).\n"
                    "5. Drilldown (needs LIBTPU_INIT_ARGS --xla_jf_dump_to=<dir>\n"
                    "   --xla_jf_dump_llo_text=true — NOT --xla_dump_to!):\n"
                    "   `list_llo_programs` -> `get_llo_static_utilization` (hot\n"
                    "   address ranges) -> `get_llo_bundles(address_range=...)`.\n"
                    "   Tensor Core trace markers carry the same 0x addresses.\n"
                    "6. `get_custom_call_mlir(kernel=...)` — structural audit: did\n"
                    "   the compiler tile/fuse as the plan intended?\n\n"
                    "% util = LLO static slot occupancy over measured windows (not\n"
                    "raw HW counters; runtime counter sampling is TPU v7+ only and\n"
                    "silently absent on earlier gens). See docs/KERNEL_PROFILING.md.\n\n"
